...
namespace LinqTasks
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var query6 = buyers.Where(b => b.Cars.All(c => c.ReleaseDate.Year >= 2019))
            ...
            var query7 = buyers.GroupBy(b => b.Cars.Select(c => c.Model))
                .OrderByDescending(d => d.Select(v => v.FirstName.Count()))
                .Select(r => new { Model = r.Key, Count = r.Select(v => v.FirstName.Count()) });
            ...
            var query8 = buyers.GroupBy(b => b.Address.Street)
                .Select(r => new { Street = r.Key, Models = string.Join(", ", r.Select(c => c.Cars.OrderByDescending(f => f.Model.Count()))) });

            /* 9. Полные имена, зарплаты и полный адрес покупателей не имеющих автомобилей */
            var query9 = buyers.Where(b => b.Cars.Count() < 1)
            ...
            var query10 = buyers.Where(b => b.Cars.Count() < 1)
                  .Where(b => b.Cars.Select(c => c.Price < b.Salary*6*20%))//<-- не совсем верно, знаю, я малость запуталась
                  .Select(r => r.LastName);
        }
    }
}
